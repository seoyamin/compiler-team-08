...
void defineIdentType(const char *type, const char *identifier, const char *identifierReturnType); 
%}
...
type_specifier 		: TINT												
		 			| TVOID												;
function_name 		: TIDENT											{defineIdentType("function name", identReturnType, identStr);};
					| TERROR											
...
declarator 			: TIDENT												{defineIdentType("integer scalar variable", NULL, identStr);}
					| TERROR												
					| TIDENT TLEFTBRACKET TNUMBER TRIGHTBRACKET			{defineIdentType("integer array variable", NULL, identStr);}
					| TERROR TLEFTBRACKET TNUMBER TRIGHTBRACKET			
					| TIDENT TLEFTBRACKET TRIGHTBRACKET					{defineIdentType("integer array variable", NULL, identStr);}
...
primary_exp 		: TIDENT												{defineIdentType("primary expression", NULL, identStr);}
					| TERROR												
					| TNUMBER												
...
void defineIdentType(const char *type, const char *returntype, const char *identifier)
{
	//printf("\n\n******  Inside the defineIdentType()  ******\n");
	//printf("type: %s, returntype: %s, identifier: %s \n\n", type, returntype, identifier);
	...
	if (HT[hashcode] != NULL) {  
		...
		// 함수 identifier의 경우 return type을 저장
		if(returntype != NULL) {
			here->returntype = returntype;	
			printf("_________ %s %s %s\n", identifier, returntype, here->returntype);
		}
	}
}
